...
import hashlib
from functools import wraps # Importamos wraps para crear el decorador
...
# Definir categorías y contenido
LEVELS = {
    'principiante': {
        'name': 'Principiante',
        'videos': [
...
def hash_password(password):
    """Función simple para hashear contraseñas (usar librería más segura como bcrypt en producción)"""
...
# Aplicamos el decorador @login_required a la ruta que muestra los videos
...
def login():
    if request.method == 'POST':
        ...
        try:
            response = supabase.table('users').select('*').eq('UserName', username).eq('UserPassword', hashed_password).execute()
            user = response.data
            if user:
